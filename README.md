...
| 🗂️ **Metadata Filtering** | Scope any query to specific files via `POST /ask/filtered` to eliminate cross-document noise. |
...
| 🏗️ **Modular Architecture** | Clean layer separation across `connectors/`, `processing/`, `embedding/`, `search/`, and `api/` — swap any component without touching the rest of the pipeline. |
